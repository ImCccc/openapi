...
export const prettierFile = (content: string): [string, boolean] => {
  ...
  try {
    ...
    result = prettyConfig
      ? prettier.format(content, { ...prettyConfig, parser: "typescript" })
      : content;
  } catch (error) {
    hasError = true;
  }
...
export const formatApiInfo = (apiInfo: Record<string, any>): any => {
  ...
  apiInfo.schema.tags = apiInfo.schema.tags.map(
...
export function formatParamsForYFH(
  ...
  Object.keys(params).forEach((name) => {
    ...
    if (nameListLength === 1) {
      ...
      paramsObject[childKey] = combineParams(
        childKey,
        key_child_key,
        prop,
        paramsObject
      ...
      // key.n.child_key
      if (
        nameList[nameListLength - 2] === "n" ||
        nameList[nameListLength - 2] === "m"
      ) {
        const child_key = camelCase(nameList.pop());
        nameList.pop();
        key = nameList.join(".");
        paramsObject[key] = combineParams(
          key,
          child_key,
          prop,
          paramsObject,
        ...
        if (child_key === "n" || child_key === "m") {
          // .n.key.m
          if (
            nameList[nameList.length - 2] === "n" ||
            nameList[nameList.length - 2] === "m"
          ) {
  ...
  const hasInvoke =
    Object.keys(paramsObject).filter((param) => param.includes(".")).length > 0;
...
function combineParams(
  ...
  if (paramsObject[key]) {
    const child_type = `{${child_key}:${prop.type}${typeSuffix}, ${paramsObject[
      key
    ].type.slice(1)}`;
    paramsObject[key] = {
      ...paramsObject[key],
